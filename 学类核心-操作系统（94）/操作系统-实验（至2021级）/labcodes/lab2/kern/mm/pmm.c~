...
pte_t *get_pte(pde_t *pgdir, uintptr_t la, bool create) {
    pde_t *pdep = &pgdir[PDX(la)];              //通过PDX（la）找到页目录项索引
    if (!*pdep&PTE_P) {                         //判断物理页是否存在
        if(!create) return NULL;                //create=0时直接返回，即不创建
        else{                         
            struct Page* page=alloc_page();     //分配一页用于存放二级页表
            if(page==NULL) return NULL;
            set_page_ref(page,1);               //设置这个页被引用
            uintptr_t pte_pa=page2pa(page);     //获得这个页的地址，用于填写到pde目录项中
            memset(KADDR(pte_pa),0,PGSIZE);     //清除页面内容
            *pdep=pte_pa | PTE_P | PTE_W | PTE_U;   //重置目录项
        }
    }
