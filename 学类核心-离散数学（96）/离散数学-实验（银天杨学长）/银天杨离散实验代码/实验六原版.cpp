...
{
	//集合保存在一个字符串，显示时在各元素间加入逗号，及返回长度
	int i=0;
	if(n>0){printf("<%c,%c>",a[0][0],a[0][1]);} 
	for(i=1;i<n;i++) printf("<%c,%c>",a[i][0],a[i][1]);
	printf("\n");
	return n;
}
...
void rela2matrix(char a[],char R[][3],int nR,int M[][N])
{
	//关系矩阵的行列，等于基本元素即数组a中当前元素个数
	 ...
	 //寻找关系R中每个序偶元素对应的序号，从而在数组M的指定位置成1
	 for(i=0;i<nR;i++)
	 {
	 	for(j=0;j<na;j++)
	 	{
	 		if(R[i][0]==a[j]) {k=j;break; }
		 }
		for(j=0;j<na;j++)
		{
			if(R[i][1]==a[j]) {m=j;break;}
		}
		M[k][m]=1;
	  } 
}
