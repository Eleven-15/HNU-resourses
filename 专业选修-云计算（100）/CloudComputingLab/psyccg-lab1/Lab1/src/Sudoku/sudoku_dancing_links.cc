...
// soduku_dancing_links算法需要的定义
// soduku_dancing_links.h
struct Node;
typedef Node Column;
struct Node
{
...
{
public:
    struct answer solve_ans()
    {
        solve();
        char ans[N + 1];
        for (int cell = 0; cell < N; ++cell)
        {
            ans[cell] = inout[cell] + '0';
            assert('0' <= ans[cell] && ans[cell] <= NUM + '0');
        }
        ans[N] = '\0';
        struct answer answer(ans);
        return answer;
    }

    Dance(const char *puzzle)
    {
        cur_node_ = 0;
        for (int cell = 0; cell < N; ++cell)
        {
            inout[cell] = puzzle[cell] - '0';
            assert(0 <= inout[cell] && inout[cell] <= NUM);
            if (DEBUG)
                printf("%d", inout[cell]);
        }
        if (DEBUG)
...
{
    // puzzle为读入的一组问题
    class Dance *new_dance = new Dance(puzzle);
    // 调用问题解决并且返回结果
    struct answer ans = new_dance->solve_ans();
    return ans;
}
