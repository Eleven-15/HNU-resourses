...
Buffer HttpResponse::makeResponse()
{
    ...
    if (statusCode_ == 400)
    {
        doErrorResponse(output, "wsver can't parse the message");
        return output;
    }
    ...
    {
        statusCode_ = 404;
        doErrorResponse(output, "wsver can't find the file");
        return output;
    }
    // 权限错误
    ...
    {
        statusCode_ = 403;
        doErrorResponse(output, "wsver can't read the file");
        return output;
    }
...
void HttpResponse::doErrorResponse(Buffer &output, std::string message)
{
    ...
    body += "<html><title>wsver Error</title>";
    body += "<body bgcolor=\"ffffff\">";
    body += std::to_string(statusCode_) + " : " + itr->second + "\n";
    body += "<p>" + message + "</p>";
    body += "<hr><em>wsver web server</em></body></html>";
