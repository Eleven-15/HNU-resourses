...
HttpServer::HttpServer(std::string ip, int port, int numThread, std::string proxy)
    : port_(port),
      ip_(ip),
      proxy_(proxy),
...
void HttpServer::__acceptConnection()
{
    while (1)
    {
        ...
        // SOCK_NONBLOCK 非阻塞模式 在没有数据可读取或写入的情况下，套接字的读写操作将立即返回而不阻塞进程
        // SOCK_CLOEXEC 设置新套接字为 close-on-exec 标志。在执行 exec 系列函数时，新套接字将被自动关闭，以避免在子进程中继承套接字造成的资源泄漏
